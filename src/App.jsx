          ...
          <div className="entries-list">
            {entries.map((entry) => (
              <article className="entry" key={entry.title}>
                <time className={`date-block ${entry.color}`}><strong>{entry.day}</strong><span>{entry.month}</span></time>
      ...
      {modalOpen && (
        <div className="modal-backdrop" role="presentation" onMouseDown={() => setModalOpen(false)}>
          <section className="record-modal" role="dialog" aria-modal="true" aria-labelledby="record-title" onMouseDown={(event) => event.stopPropagation()}>
            <button className="modal-close" aria-label="Close recording" onClick={() => { setModalOpen(false); setRecording(false); }}><Icon name="close" size={28} /></button>
            ...
            <button className="record-button large" onClick={() => setRecording((value) => !value)}>{recording ? <><Icon name="pause" /> Pause</> : <><Icon name="mic" /> Start recording</>}</button>
          </section>
        </div>
      )}
    </div>
